...
VERCEL_URL = "https://gamer520-daily.vercel.app"
    ...
    for page in range(1, 6):
        ...
        try:
            ...
            if not page_time:
                ...
                if time_matches:
                    page_time = time_matches[-1]  # 取最后一个（通常是页面底部）
        ...
        <a class="game-card" href="${{game.link}}" target="_blank" style="animation-delay:${{i*0.03}}s">
            <div class="game-cover-wrapper">
                <img class="game-cover" src="${{game.cover || ''}}" alt="${{game.name}}" loading="lazy"
                     ...
                     onclick="event.preventDefault();openLightbox('${{game.cover}}', '${{game.name}}')">
...
async function handleRefresh() {{
    ...
    try {{
        ...
        const resp = await fetch(GITHUB_RAW + '?t=' + ts);
        ...
        if (m) {{
            ...
            showToast('已刷新，共 ' + GAMES_DATA.length + ' 款游戏');
        }} else {{
            showToast('未找到数据');
        }}
    }} catch(e) {{
        ...
        card = {
            ...
            "elements": [
                ...
                {"tag": "action", "actions": [
                    ...
                    {"tag": "button", "text": {"tag": "plain_text", "content": "🌐 国际访问"}, "type": "default", "url": VERCEL_URL}
                ]},
